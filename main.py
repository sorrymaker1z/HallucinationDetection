...
def main():
    ...
    print("neg_features and pos_features...")
...
def evaluate(dataset, tokenizer, model, C_M, C_FA, C_retrieve, P0, neg_features, pos_features, args):
    ...
    for passage in tqdm(dataset):
        ...
        for sentence in passage['gpt3_sentences']:
            ...
            hypothesis_file_name = args.decomposed_file_path + '/decomposed_' + str(passage_index) + '_' + str(
                sentence_index) + '.txt'
    ...
    log_txt = "./log/log_cm_" + str(C_M) + "_cfa_" + str(C_FA) + ".txt"
    with open(log_txt, "w") as log_file:
        log_file.write(f"acc: {1 - wrong_num / total_num}\n")

        log_file.write(f"Non_fact_auc_precision_recall: {Non_fact_auc_precision_recall}\n")

        log_file.write(f"fact_auc_precision_recall: {fact_auc_precision_recall}\n")

        log_file.write(
            f"avg_sentence_search_time: {sum(hypothesis_search_time_list) / len(hypothesis_search_time_list)}\n")

        log_file.write(f"pearson: {scipy.stats.pearsonr(passage_golden_label_list, passage_test_label_list)[0]}\n")

        log_file.write(f"Spearman: {scipy.stats.spearmanr(passage_golden_label_list, passage_test_label_list)[0]}\n")

        log_file.write(
            f"hypothesis_avg_search_time: {sum(sentence_search_time_list) / len(sentence_search_time_list)}\n")
